...
fn create_replenishment_service(pool: &PgPool) -> PgReplenishmentService {
...
async fn setup_test_tenant_product_warehouse(pool: &PgPool) -> (Uuid, Uuid, Uuid) {
    ...
    .bind(tenant_id)
    .bind("Reorder Test Tenant")
    .bind(format!("reorder-test-{}", tenant_id))
    .execute(pool)
    .await
    ...
    .bind(product_id)
    .bind(tenant_id)
    .bind(format!("REORDER-{}", Uuid::now_v7()))
    .bind("Reorder Test Product")
    .execute(pool)
    .await
...
async fn create_inventory_level(pool: &PgPool, tenant_id: Uuid, product_id: Uuid, warehouse_id: Uuid, quantity: i64) {
    // Use simple INSERT since location_id is NULL and the constraint includes location_id
    // First try to delete any existing record, then insert
    let _ = sqlx::query(
...
mod reorder_quantity_tests {
    ...
    async fn test_reorder_quantity_at_least_min() {
        ...
        assert!(result.needs_replenishment);
        // Even though max - current = 55, the logic should suggest enough to bring near max
        // Implementation suggests max - current = 100 - 45 = 55, which is >= min(30)
        assert!(result.suggested_order_quantity >= 30, "Should order at least min_quantity");
...
mod safety_stock_tests {
    ...
    async fn test_lead_time_in_rule_creation() {
        ...
        let rule = CreateReorderRule {
            product_id,
            warehouse_id: Some(warehouse_id),
            reorder_point: 100, // Higher reorder point for longer lead time
            min_quantity: 50,
            max_quantity: 200,
            lead_time_days: 14,
            safety_stock: 20,
...
mod reorder_edge_cases {
    ...
    async fn test_replenishment_check_no_rule_returns_not_found() {
        ...
        // Don't create any reorder rule
        // Try to check replenishment
        let result = service
            .check_product_replenishment(tenant_id, product_id, Some(warehouse_id))
            .await;
