...
/// PostgreSQL implementation of DeliveryOrderRepository
...
impl DeliveryOrderRepository for PgDeliveryOrderRepository {
    async fn create(&self, delivery_order: &DeliveryOrder) -> Result<DeliveryOrder> {
        let result = sqlx::query_as!(
            ...
            INSERT INTO delivery_orders (
                delivery_id, tenant_id, delivery_number, reference_number, warehouse_id,
                order_id, customer_id, status, delivery_date, expected_ship_date,
                actual_ship_date, shipping_method, carrier, tracking_number,
                shipping_cost, notes, created_by, total_quantity, total_value, currency_code,
                created_at, updated_at
            ) VALUES (
                $1, $2, $3, $4, $5, $6, $7, $8::delivery_order_status, $9, $10,
    ...
    async fn update(&self, delivery_order: &DeliveryOrder) -> Result<DeliveryOrder> {
        let result = sqlx::query_as!(
            ...
            UPDATE delivery_orders SET
                delivery_number = $3, reference_number = $4, warehouse_id = $5,
                order_id = $6, customer_id = $7, status = $8::delivery_order_status,
                delivery_date = $9, expected_ship_date = $10, actual_ship_date = $11,
                shipping_method = $12, carrier = $13, tracking_number = $14,
                shipping_cost = $15, notes = $16, total_quantity = $17,
                total_value = $18, currency_code = $19, updated_at = $20
            WHERE tenant_id = $1 AND delivery_id = $2 AND deleted_at IS NULL
            RETURNING *
            "#,
            delivery_order.tenant_id,
    ...
    async fn delete(&self, tenant_id: Uuid, delivery_id: Uuid) -> Result<()> {
        sqlx::query!(
            r#"
            UPDATE delivery_orders SET deleted_at = NOW()
            WHERE tenant_id = $1 AND delivery_id = $2 AND deleted_at IS NULL
            "#,
            tenant_id,
            delivery_id,
...
/// PostgreSQL implementation of DeliveryOrderItemRepository
...
impl DeliveryOrderItemRepository for PgDeliveryOrderItemRepository {
    async fn create(&self, item: &DeliveryOrderItem) -> Result<DeliveryOrderItem> {
        let result = sqlx::query_as!(
            ...
            INSERT INTO delivery_order_items (
                delivery_item_id, tenant_id, delivery_id, product_id, ordered_quantity,
                picked_quantity, delivered_quantity, unit_price, line_total,
                batch_number, expiry_date, uom_id, notes, created_at, updated_at
            ) VALUES (
    ...
    async fn update(&self, item: &DeliveryOrderItem) -> Result<DeliveryOrderItem> {
        let result = sqlx::query_as!(
            ...
            UPDATE delivery_order_items SET
                ...
                unit_price = $7, line_total = $8, batch_number = $9, expiry_date = $10,
                uom_id = $11, notes = $12, updated_at = $13
    ...
    async fn delete(&self, tenant_id: Uuid, delivery_item_id: Uuid) -> Result<()> {
        sqlx::query!(
            r#"
            UPDATE delivery_order_items SET deleted_at = NOW()
...
impl InventoryRepository for PgInventoryRepository {
    async fn reserve_stock(&self, tenant_id: Uuid, product_id: Uuid, quantity: i32) -> Result<()> {
        ...
        // This assumes inventory_levels table has available_quantity column
        sqlx::query!(
            r#"
