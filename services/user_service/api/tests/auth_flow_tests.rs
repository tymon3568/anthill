...
async fn test_complete_registration_to_authenticated_request_flow() {
    ...
    let (status, register_response) = make_request(
        &app,
        "POST",
    ...
    let (status, profile_response) = make_request(
        &app,
        "GET",
        "/api/v1/profile",
        None,
    ...
    let (status, update_response) = make_request(
        &app,
        "PUT",
        "/api/v1/profile",
        Some(update_payload),
...
async fn test_login_flow_with_token_refresh() {
    ...
    let (status, refresh_response) = make_request(
        &app,
        "POST",
        "/api/v1/auth/refresh",
...
async fn test_rbac_flow_manager_permissions() {
    ...
    let _manager_id = db.create_user(
        tenant_id,
        "manager@rbac.com",
        &manager_password,
        "manager",
