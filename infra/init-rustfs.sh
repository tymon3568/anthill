...
# RustFS is S3-compatible, so we use mc (MinIO Client) for bucket operations
...
while [ $retry_count -lt $max_retries ]; do
  if mc alias set rustfs http://localhost:9000 rustfsadmin rustfsadmin --api S3v4 2>/dev/null; then
