...
# 🎨 UI Design Guidelines - Frappe UI Style
...
- ✅ **Minimal shadows** - Remove most shadow effects for cleaner look
- ✅ **Simple rounded corners** - Use `rounded` or `rounded-lg`, avoid `rounded-xl`
- ✅ **Gray-based color scheme** - Prefer simple grays over complex color systems
- ✅ **Clean focus states** - Simple border changes, no complex rings
- ✅ **Compact spacing** - Reduce padding and margins for tighter layouts
...
// ❌ WRONG: Avoid shadcn default style
...
/* ❌ WRONG: Avoid complex color systems */
...
# Agent & Contributor Instructions — ALWAYS Use Bun
...
**❌ NEVER use:** `npm install`, `npm add`, `pnpm add`
**✅ ALWAYS use:** `bun install`, `bun add`
...
**❌ NEVER use:** `npm run dev`, `npm run build`
**✅ ALWAYS use:** `bun run dev`, `bun run build`
